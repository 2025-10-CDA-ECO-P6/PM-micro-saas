...
Le détail complet des arbitrages de périmètre — critères de sortie, risques, dépendances par use case — fait foi dans `docs/conception/vision/moscow.md`. Cette section n'en constitue qu'une synthèse.
...
### 5.4 Point non tranché — double référent du symbole « J1 »

Le corpus porte une incohérence apparente, signalée mais non tranchée : le symbole « J1 » a deux référents distincts selon la source. La décision d'architecture actant le périmètre du MVP nomme J1 le jalon **local-only** (le jalon décrit en 5.1 ci-dessus). Une autre décision d'architecture, portant sur la conformité RGPD et le modèle d'autorisation, mentionne un « jalon J1 (Vague 3) » associé à une préoccupation **cloud**. `[À RATIFIER — opérateur]`. Ce document retient provisoirement J1 = local-only, conformément à l'usage le plus densément documenté dans le corpus, sans que cela ne constitue une résolution de la divergence.
...
**Vision**
- `conception/vision/vision-produit.md` — positionnement produit, hypothèses de validation, modèle de monétisation.
- `conception/vision/moscow.md` — priorisation détaillée du périmètre (Must/Should/Could/Won't Have).

**Cahier des charges**
- `cahier-des-charges.md` — document officiel de référence pour la construction du MVP, consolidant le corpus de conception.

**Planning**
- `planning/roadmap-entree-build.md` — séquence technique d'entrée en build (jalons J0-J3, gates, préalables bloquants).
- `planning/roadmap-produit.md` — trajectoire de valeur produit (paliers, jalon de validation, extensions par signal).
- `planning/registre-risques.md` — registre de risques consolidé (quatre axes technique/produit/juridique/délai-coût, matrice de criticité, vue par hypothèse H1-H5, vue par gate).

**Business**
- `business/note-business-gtm.md` — méthode de calcul du coût d'infrastructure, du dimensionnement de marché et de la stratégie go-to-market.
