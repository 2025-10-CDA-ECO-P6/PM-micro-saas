...
| 5 | Facette RGPD — Option A « reclaim-in-place » (coquille non vérifiée évincée) | ADR-015 §2.3 (l.121-145, 331-332, 355) — routé du Lot 7 | à valider juriste |
...
## Axe 5 — Facette RGPD de l'Option A « reclaim-in-place » *(routé du Lot 7)*
...
- ADR-015, tableau de synthèse, l.331-332 : « Reclaim-in-place (§2.3) — facette RGPD (sort du contenu éventuel de la coquille non vérifiée évincée) | RGPD | Lot 14 ».
- ADR-015, l.355 (*[À TRANCHER — à ratifier opérateur]*) : résume la résolution proposée et le double routage (facette RGPD → Lot 14 ; opération de domaine dédiée → build B1.5).
- Provenance : `docs/BACKLOG.md`, Lot 7 (item routé), l.205.
...
Ce cadrage doit être porté comme **jalon** dans la section *Préalables & jalons de build* du backlog (`docs/BACKLOG.md`), au même titre que les autres préalables non réalisables dans un repo documentaire (confirmation des 8 ADR pré-implémentation, ordre de build C#-first, gate de validation marché, etc.). Le backlog référence déjà ce cadrage (Lot 14, item consolidé) et la ligne *Préalables & jalons de build* correspondante (« Réaliser la validation juridique avant lancement EU — cf. Lot 14 ») ; ce document en est le contenu détaillé, pas un doublon de suivi.
...
- [`docs/juridique/dpa-skeleton.md`](./dpa-skeleton.md) — squelette de sections DPA (axe 2).
- `docs/BACKLOG.md` — Lot 14 (item consolidé) et Lot 7 (item routé, axe 5) ; section *Préalables & jalons de build*.
