...
**Recoupement backlog** : cette roadmap recoupe l'item *« Réaliser la validation juridique avant lancement EU »* des « Préalables & jalons de build » (`docs/BACKLOG.md`) et les items du Lot 14 (« RGPD & juridique ») du même backlog. Elle ne s'y substitue pas — elle en explicite le séquencement.
...
**Contenu attendu** : ADR-018 consolide explicitement 5 axes de validation juridique pré-lancement EU dans un document dédié produit sous `docs/juridique/cadrage-validation-pre-lancement-eu.md`. Les 5 axes identifiés par la conception sont :
...
**Sources conception** : ADR-013 §5 ; ADR-013 Conséquences (« la roadmap juridique doit inclure la rédaction du DPA ») ; ADR-018 *Conformité conçue, non certifiée* point 2 ; `docs/BACKLOG.md` Lot 14, item « Produire le DPA + roadmap juridique ».
...
## Recoupement avec les « Préalables & jalons de build »

Le backlog (`docs/BACKLOG.md`, section « Préalables & jalons de build ») liste, parmi les 8 préalables non réalisables dans ce repo documentaire :

- *« Réaliser la validation juridique avant lancement EU (cf. Lot 14) »* — couvert par les jalons 1-2 ci-dessus.
- *« F-04 (base légale invités) avant EU »* (préalable bloquant MVP) — couvert par le jalon 1.

Le Lot 14 du backlog (« RGPD & juridique ») liste des items complémentaires non repris comme jalons de cette roadmap (hors périmètre du DPA/politique de confidentialité au sens strict) mais dont l'issue peut affecter la rédaction des jalons 3-4 :
...
- Facette RGPD de l'Option A « reclaim-in-place » (routé du Lot 7).

Cette roadmap ne reprend pas ces items en jalons distincts — ils restent suivis dans le backlog — mais signale leur dépendance potentielle sur les jalons 3 et 4 ci-dessus (une procédure de droits non stabilisée peut faire évoluer la politique de confidentialité après une première rédaction).
...
| Recoupement backlog | `docs/BACKLOG.md`, « Préalables & jalons de build » et Lot 14 |
