...
> La propriété d'un espace (y compris `SpaceType.PERSONAL`) naît à la création de compte ; en mode local, un espace est un conteneur sans `ownerId` (cohérent ADR-017/ADR-018). Le détail de ce régime relève de W5/ADR-016-017.
...
3. `User.Delete()` est bloqué si l'utilisateur possède des espaces avec des membres actifs (vérification applicative — Space Management est interrogé via un contrat applicatif avant l'exécution de la commande).

   > **Point à préciser (ADR-018, W2)** : un espace `PERSONAL` est mono-membre par construction (son propriétaire) ; la sémantique de blocage de `User.Delete()` doit exclure le cas `PERSONAL` — voir ADR-018 §Conséquences (purge inconditionnelle de l'espace `PERSONAL` à `UserDeleted`). En l'état, l'invariant 3 lu littéralement bloquerait toute suppression de compte.
...
|---|---|---|
| `UserRegistered` | `User.Register()` | Application (email de bienvenue), Space Management (si conversion depuis GuestAccess) |
...
| `UserDeleted` | `User.Delete()` | Space Management (anonymisation des member data), Content Library si nécessaire |
