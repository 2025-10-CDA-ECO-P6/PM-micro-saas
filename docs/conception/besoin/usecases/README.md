...
| [UC-02](UC-02-creer-espace-jeu.md) | Créer un espace de jeu | MJ | 1 | Le MJ crée un espace de travail nommé — conteneur de tout le contenu, avec un minimum de friction, qu'il s'agisse d'une campagne longue ou d'un one-shot (→ UC-02). |
...
| [UC-04](UC-04-gerer-documents-espace.md) | Gérer les documents d'un espace | MJ | 1 + 2 | Le MJ capture rapidement des informations à tout moment — documents libres, notes rapides, lore ou éléments typés — sans contrainte de structure ni délai. |
...
| [UC-11](UC-11-gerer-membres-espace-partage.md) | Gérer les membres d'un espace partagé | MJ | — | Le MJ invite des membres permanents à sa campagne et génère des liens de session temporaires pour les groupes ponctuels (one-shots, conventions). |
| [UC-12](UC-12-consulter-espace-joueur.md) | Consulter son espace en tant que joueur (vue post-accès) | Joueur | — | Après avoir obtenu l'accès (via UC-09 ou UC-11), le joueur consulte sa campagne : fiche, documents `PUBLIC`, périmètre SESSION/CAMPAIGN. |
...
| ID | Use Case | Résumé |
|---|---|---|
| [UC-15](UC-15-gel-espaces-downgrade-tier.md) | Gel d'espaces au downgrade de tier | Espaces excédentaires gelés automatiquement (lecture seule) au downgrade ; dégel automatique au ré-upgrade, sans perte de données. |
