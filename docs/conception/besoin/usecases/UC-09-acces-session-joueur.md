# UC-09 — Accès joueur via lien (session ou espace partagé)
...
Permettre à un joueur d'obtenir l'accès à un espace partagé (`CAMPAIGN` ou `ONE_SHOT`) ou à une session via un lien partagé par le MJ, avec ou sans compte : soit comme invité ponctuel pour une session (sans compte, friction minimale), soit comme membre permanent de l'espace partagé (avec compte, accès persistant).
...
Le MJ partage un lien d'accès à une session (lien ponctuel) ou à son espace partagé (lien permanent) avec ses joueurs.

## Préconditions

- Un espace partagé (`CAMPAIGN` ou `ONE_SHOT`) existe — un espace `PERSONAL` est mono-membre et ne peut recevoir ni invitation ni accès invité (invariant 13 du domaine), hors périmètre d'UC-09.
- Le MJ a un compte (au minimum gratuit).
...
- Pour le chemin espace partagé permanent : le MJ a généré un lien d'espace partagé depuis le panneau membres (UC-11).

> **Note.** Le chemin B (« espace partagé permanent ») correspond à la portée `SPACE` d'`Invitation` et de `GuestAccess` : un régime d'accès **durable**, distinct de la portée `SESSION` (accès ponctuel, chemin A). Cette portée s'applique aussi bien à un espace `CAMPAIGN` qu'à un espace `ONE_SHOT` (glossaire, entrée `Invitation`). Le terme « espace partagé » employé dans cet UC désigne le conteneur (`CAMPAIGN` ou `ONE_SHOT`), jamais l'espace `PERSONAL` (mono-membre, invariant 13).
...
## Scénario nominal B — Accès espace partagé permanent (joueur authentifié)

1. Le MJ génère un lien d'espace partagé depuis le panneau membres (UC-11) et le partage avec le joueur.
2. Le joueur clique sur le lien.
3. S'il n'est pas encore connecté, l'application lui propose de se connecter ou de créer un compte (UC-10 pour la création de compte ; la jonction à l'espace partagé se complète dans UC-09 une fois authentifié).
4. Le joueur est connecté à son compte Haversack.
5. L'application crée un `SpaceMembership` en `PENDING` puis l'active immédiatement via `Activate()` : le lien d'invitation à portée `SPACE` généré par le MJ (UC-11) tient lieu de validation au sens de RB-09-16 — c'est l'invitation préalable du MJ, pas un second clic, qui autorise l'activation. Le cycle de domaine `PENDING → ACTIVE` est respecté sans exception.
6. Le joueur obtient un accès persistant à l'espace partagé : il peut consulter les sessions passées, les documents partagés, et son historique de personnage selon le périmètre du lien.
7. La vue obtenue une fois l'accès actif (fiche du personnage actif, documents `PUBLIC`, historique selon périmètre) relève d'**UC-12**.
...
5. Il peut ensuite rejoindre l'espace partagé via un lien d'espace partagé (scénario nominal B) ; son admission comme `Member` reste soumise au consentement du MJ (RB-09-16), indépendamment de la validation de son email.
...
- Un lien révoqué affiche un message clair sans révéler d'information sur l'espace partagé.

**Chemin B — espace partagé permanent (avec compte)**
- Un joueur authentifié suivant un lien d'espace partagé obtient un `SpaceMembership` actif dans l'espace partagé (cycle `PENDING → Activate()` respecté ; l'invitation du MJ tient lieu de validation).
