...
Pour valider le concept et justifier une suite, le MVP doit démontrer cinq hypothèses auprès des utilisateurs réels. Chaque hypothèse reçoit un seuil chiffré et un délai d'observation — ces seuils sont calibrés pour une cohorte pilote restreinte (early adopters recrutés), révisables avant le lancement de l'observation, et constituent une décision GO/NO-GO objective à l'issue de la période.
...
|---|---|---|---|---|
| **H1** | Un MJ peut créer un espace structuré et y retrouver ses informations sans friction d'onboarding (pas de compte obligatoire au démarrage). | ≥ 60 % des MJ de la cohorte pilote qui ouvrent l'application atteignent **activation préparation** (espace actif — campagne ou espace personnel — avec premiers documents créés). *(instrument précisé — décision produit du 2026-07-09, qui raffine la position ADR-018 du 2026-06-12 : le contenu de l'espace personnel sans campagne ne compte pour H1 que **partiellement**, lorsqu'il traduit un **geste structurant** — au-delà d'une capture triviale (organisation, structuration, franchissement d'un seuil) — et non de façon équivalente à une campagne créée. Seuil/critère exact du geste structurant, tranché (décision produit du 2026-09-03) : **un dossier créé, ou un document déplacé hors de « Non classés »** — deux gestes observables sans lire aucun contenu. Le seuil de documents de l'activation préparation est fixé à **trois**. Voir `moscow.md § Instrumentation de validation du MVP`.)* | 14 jours après le premier usage. | Activation préparation (mesure d'usage anonyme). |
...
> **Limite structurelle de l'instrument — à rappeler à toute lecture des résultats** (décision produit du 2026-09-03)
>
> Les compteurs d'usage sont écrits dans le store local et **ne quittent le navigateur qu'à la création d'un compte**
> (`moscow.md § Instrumentation de validation du MVP`). Ce choix préserve la garantie observable « aucune donnée envoyée
> au serveur en mode local », mais il a une conséquence qu'aucune lecture des résultats ne doit ignorer :
>
> **les MJ qui ne créent jamais de compte ne transmettent jamais leurs compteurs.** Or H1 porte précisément sur un usage
> **sans compte obligatoire**. Elle n'est donc observée que chez ceux qui convertissent — un biais en faveur des
> convertis. H5, qui mesure la conversion, en devient partiellement circulaire : son dénominateur ne contient que des
> convertis.
>
> Les entretiens utilisateurs, déjà nommés comme instrument de H3 et H4, sont le seul contrepoids disponible pour H1 et
> H5. Un seuil atteint ou manqué doit être lu avec ce biais nommé, jamais comme une mesure de la population entière.

> **Conception d'interface — Support de démonstration de H2**
> 
> La démonstration de l'hypothèse H2 s'appuie sur une conception documentée de l'interface de la vue session. Les wireframes basse-fidélité, dérivés du parcours utilisateur (UJ-UC-06) et des critères d'acceptation des user stories, constituent un livrable au périmètre de la conception. Ils sont produits (arbitrage T-09 du 2026-06-10 ; voir `docs/conception/interface/wireframes/README.md`).
...
**Condition de retour** : si les entretiens ou l'usage révèlent que le profil one-shot exclusif (Sonia — conventions, groupes changeants, catalogue de scénarios) est une part significative des utilisateurs réels ou un levier d'adoption, UC-13 est réexaminé en priorité lors d'une version ultérieure.
...
**Raison d'être produit** : le processus créatif du MJ produit du contenu (lieu, PNJ, scénario, règle maison) sans lien avec une campagne en cours. Ce contenu est de premier ordre — il appartient au MJ, pas à une campagne. Reconnaître l'espace personnel comme conteneur par défaut supprime la friction « je dois créer une campagne pour noter une idée » et est directement cohérent avec le différenciant « friction d'entrée nulle » et avec le principe « Tout est Document ». Cette décision change la définition de l'instrument H1 : « activation préparation » comptait initialement (2026-06-12) le contenu créé dans l'espace personnel de façon équivalente à une campagne créée. **Décision produit du 2026-07-09 (raffine cette position antérieure)** : ce contenu ne compte désormais que **partiellement** pour H1 — lorsqu'il traduit un **geste structurant** (au-delà d'une capture triviale : organisation, structuration, franchissement d'un seuil), non de façon équivalente à une campagne créée. Le seuil/critère exact de ce geste structurant est tranché (décision produit du 2026-09-03) : **un dossier créé, ou un document déplacé hors de « Non classés »** ; définition consolidée en §2.3.
...
- *Voie 2 — `CampaignType.PERSONAL` sans renommage* — repli viable, non retenu : dette sémantique significative (le MJ stocke son contenu dans « une campagne nommée Personnel », réintroduit le biais campagne-centré). Cette voie laisserait une `ScenarioLibrary` de pont sans résolution naturelle. La Voie 3 subsume ce problème : bibliothèque personnelle = vue filtrée sur `Document.isReusable` de l'espace PERSONAL (ADR-018).
