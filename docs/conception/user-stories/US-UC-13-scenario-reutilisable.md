...
- Le scénario source reste dans la campagne d'origine. Le `Document` dans l'espace personnel est le template de référence — la question copie vs référence vivante (Q1 ci-dessous) reste ouverte pour la modélisation détaillée.
...
1. (Partiellement répondue par ADR-018) Le scénario source dans l'espace personnel est le `Document` de référence ; l'instance est créée par `Document.Instantiate`. La question résiduelle est : au moment du marquage ("Marquer comme réutilisable"), le `Document` est-il copié dans l'espace personnel ou déplacé depuis la campagne d'origine ? Si copié : les modifications ultérieures du scénario dans la campagne n'affectent pas le template. Si déplacé : le scénario disparaît de la campagne d'origine. À trancher lors de la modélisation détaillée.
...
5. Lors d'une instanciation via `Document.Instantiate`, quels `Document` sont inclus dans l'instance ? Seulement ceux liés au scénario, ou aussi les documents liés aux PNJ et lieux référencés par le scénario ?
