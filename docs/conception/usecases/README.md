# Use Cases — Index

> Contexte produit, positionnement MVP et acteurs → [vision-produit.md](../vision-produit.md)
>
> Priorisation MoSCoW → [moscow.md](../moscow.md)

---

## Use Cases MVP

| ID | Use Case | Acteur principal | MoSCoW |
|---|---|---|---|
| [UC-00](UC-00-inscription-connexion.md) | S'inscrire et gérer son compte | MJ / Joueur | Must Have |
| [UC-01](UC-01-creer-campagne.md) | Créer et configurer une campagne | MJ | Must Have |
| [UC-02](UC-02-structurer-scenario.md) | Structurer un scénario | MJ | Must Have |
| [UC-03](UC-03-gerer-pnj.md) | Créer et gérer des PNJ | MJ | Should Have |
| [UC-04](UC-04-gerer-notes-mj.md) | Gérer les notes MJ | MJ | Must Have |
| [UC-05](UC-05-preparer-session.md) | Préparer une session de jeu | MJ | Should Have |
| [UC-06](UC-06-vue-session.md) | Utiliser la vue session | MJ | Must Have |
| [UC-07](UC-07-fiche-personnage.md) | Créer et gérer une fiche personnage | MJ / Joueur | Should Have |
| [UC-08](UC-08-inventaire.md) | Gérer l'inventaire d'un personnage | Joueur / MJ | Could Have |
| [UC-09](UC-09-partager-information.md) | Partager une information aux joueurs | MJ | Should Have |
| [UC-10](UC-10-rejoindre-campagne.md) | Rejoindre une campagne ou une session | Joueur | Should Have |
| [UC-11](UC-11-recherche.md) | Rechercher rapidement une information | MJ | Should Have |
| [UC-12](UC-12-cloturer-session.md) | Clôturer une session et préparer la suite | MJ | Could Have |
| [UC-13](UC-13-notes-personnelles-joueur.md) | Gérer ses notes personnelles (joueur) | Joueur | Could Have |
| [UC-14](UC-14-creation-volee-session.md) | Créer un élément à la volée en session | MJ | Should Have |
| [UC-15](UC-15-gerer-membres-campagne.md) | Gérer les membres d'une campagne | MJ | Should Have |
| [UC-16](UC-16-gerer-documents-lore.md) | Gérer les documents de lore | MJ | Could Have |
| [UC-17](UC-17-organiser-dossiers.md) | Organiser le contenu en dossiers | MJ | Should Have |
| [UC-18](UC-18-synchroniser-template.md) | Synchroniser un document avec son template | MJ | Could Have |

---

## Vision long terme

- [Use Cases hors MVP](UC-HORS-MVP.md) — UC-F01 à UC-F04
