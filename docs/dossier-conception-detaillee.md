...
Ce document n'est **ni une nouvelle décision de conception, ni une décision d'architecture** : il ne prépare pas de travail d'implémentation au sens où l'entendent les specs pré-build, et il ne remplace aucun ADR. Pour l'historique et le statut de chaque décision, voir le [registre ADR](architecture/decisions/README.md) ; pour une découverte dans l'ordre pédagogique, voir l'[index d'architecture](architecture/ArchitectureIndex.md) ; pour la vue transverse par préoccupation technique, voir [07-architecture-detaillee.md](architecture/07-architecture-detaillee.md). L'audience visée est l'équipe de conception elle-même et tout relecteur qui aborde ce corpus à l'entrée en build.
...
- [Conventions de lecture](#conventions-de-lecture)
  ...
  - [Discipline « cadre fidèle »](#discipline--cadre-fidèle)
...
Ce document ne contient aucun vocabulaire d'outil d'orchestration ni de nomenclature de phasage interne (pas de « Vague N », pas de jalon numéroté hors contexte). Un jalon éventuellement mentionné se nomme par son **contenu observable** — par exemple « local-only » ou « cloud + migration » — jamais par un identifiant de vague ou de session de travail.
...
- **Vue transverse par préoccupation technique** (persistance, sécurité, RGPD, temps réel, mode local — organisée par sujet plutôt que par module) → [07-architecture-detaillee.md](architecture/07-architecture-detaillee.md).
- **Registre des décisions** (statut, nature, date de chaque ADR) → [decisions/README.md](architecture/decisions/README.md).
- **Découverte pédagogique** (ordre de lecture conseillé pour qui rejoint le projet) → [ArchitectureIndex.md](architecture/ArchitectureIndex.md).
...
**Ancrage UC.** [UC-10](conception/usecases/UC-10-compte-cloud.md) §A4 — « Suppression du compte (droit à l'effacement RGPD) ».
...
**Ancrage UC.** Accès aux ressources d'un espace — [UC-04](conception/usecases/UC-04-gerer-documents-campagne.md) (documents), [UC-06](conception/usecases/UC-06-vue-session.md) (vue session), [UC-09](conception/usecases/UC-09-acces-session-joueur.md) (accès joueur/invité), [UC-12](conception/usecases/UC-12-rejoindre-campagne.md) (vue joueur).
...
**Ancrage UC.** [UC-01](conception/usecases/UC-01-mode-local-sans-compte.md) (mode local) et [UC-10](conception/usecases/UC-10-compte-cloud.md) (migration locale→cloud).
...
sequenceDiagram
    ...
    Browser->>Proj: store local → payload (quasi-identitaire)
...
| Projection `store → payload` quasi-identitaire | ADR-016 §1.4 ; ADR-017 §1.2 | Couture stable entre store interne et contrat de migration |
...
Ce module couvre trois use cases : [UC-01](conception/usecases/UC-01-mode-local-sans-compte.md) (mode local — en creux, Identity & Access n'y intervient pas), [UC-10](conception/usecases/UC-10-compte-cloud.md) (création de compte, connexion, opérations sensibles, effacement) et [UC-11](conception/usecases/UC-11-gerer-membres-campagne.md) (dont la facette « conversion d'un invité en compte » touche Identity & Access côté création du `User`). Deux des flux ci-dessous (3.b, 3.c) sont inter-modules ou transverses : ils renvoient à leur composition complète en §2 plutôt que d'être re-dessinés.
...
**Ancrage UC.** [UC-10](conception/usecases/UC-10-compte-cloud.md), scénarios nominaux « Inscription » et règle métier « Validation de l'adresse de messagerie » — la validation n'est pas bloquante à la connexion mais requise avant toute opération sensible (modification d'email, de mot de passe, liaison fédérée, demande d'effacement).
...
**Ancrage UC.** [UC-10](conception/usecases/UC-10-compte-cloud.md), scénario nominal « Connexion via un fournisseur d'identité externe » — trois branches exhaustives selon RB-10-08 (adresse vérifiée / non vérifiée / aucun compte existant).
...
Ce module couvre cinq use cases : [UC-02](conception/usecases/UC-02-creer-espace-jeu.md) (créer un espace), [UC-11](conception/usecases/UC-11-gerer-membres-campagne.md) (gérer les membres), [UC-09](conception/usecases/UC-09-acces-session-joueur.md) (accès invité `GuestAccess`), [UC-12](conception/usecases/UC-12-rejoindre-campagne.md) (vue joueur post-accès) et [UC-15](conception/usecases/UC-15-gel-campagnes-downgrade-tier.md) (gel/dégel au changement de tier). Deux des points ci-dessous (4.d, 4.e) sont transverses et renvoient à leur composition complète en §2.
...
**Ancrage UC.** [UC-02](conception/usecases/UC-02-creer-espace-jeu.md), scénario nominal (création de campagne / one-shot) ; l'espace `PERSONAL` **préexiste** et n'est pas créé via ce parcours (UC-02 § Contexte — Ontologie des types d'espace) — sa création automatique est ancrée sur la création de compte ([UC-10](conception/usecases/UC-10-compte-cloud.md), traité en §3.a).
...
**Ancrage UC.** [UC-11](conception/usecases/UC-11-gerer-membres-campagne.md) (génération et administration des accès côté MJ — propriétaire unique des données `Invitation`/`SpaceMembership`) ; [UC-09](conception/usecases/UC-09-acces-session-joueur.md) (octroi d'accès côté joueur, propriétaire du cycle de vie `GuestAccess`).
...
**Ancrage UC.** [UC-15](conception/usecases/UC-15-gel-campagnes-downgrade-tier.md) — le use case se déclare lui-même **amont source-de-vérité** de ce comportement, le domaine (règles 6/11) en étant l'aval documenté (UC-15 § Note de positionnement d'autorité).
...
| Quota chiffré — autorité déplacée | UC-15 § Contexte ; ADR-005 (statut « fusionnée ») | Valeur non reprise ici (discipline cadre fidèle) |
...
Space Management est la **racine** de la cascade RGPD de suppression d'espace : `Space.Delete()` (space-management.md § Space, méthode `Delete()`) émet l'événement qui déclenche l'ensemble de la saga développée en **§2.1**. L'ancrage UC de ce flux reste l'angle mort déjà signalé en §2.1 — [UC-02](conception/usecases/UC-02-creer-espace-jeu.md) couvre le cycle de création/archivage mais s'arrête avant la suppression express, et le droit du propriétaire à supprimer son espace est posé directement par ADR-010 sans UC porteur explicite. Ce point n'est pas re-tranché ici. Le diagramme, les règles de gestion et le tableau des maillons sont entièrement portés par §2.1 — non redessinés.

### 4.5 — Modèle du principal et appartenance P1 (flux transverse)

#### 4.e — Modèle du principal et appartenance

**Ancrage UC.** [UC-04](conception/usecases/UC-04-gerer-documents-campagne.md), [UC-06](conception/usecases/UC-06-vue-session.md), [UC-09](conception/usecases/UC-09-acces-session-joueur.md), [UC-12](conception/usecases/UC-12-rejoindre-campagne.md).
...
Ce module couvre cinq use cases : [UC-03](conception/usecases/UC-03-structurer-scenario.md) (structurer un scénario), [UC-04](conception/usecases/UC-04-gerer-documents-campagne.md) (gérer les documents), [UC-05](conception/usecases/UC-05-organiser-dossiers.md) (organiser par dossiers), [UC-13](conception/usecases/UC-13-scenario-reutilisable.md) (scénario réutilisable) et [UC-14](conception/usecases/UC-14-recherche.md) (recherche). Un volet du point 5.e (visibilité domaine `Document.CanBeReadBy`, P4) est transverse et renvoie à sa composition complète en §2.3.

### 5.1 — UC-03 / UC-04 : cycle de vie du `Document`

#### 5.a — CRUD `Document` + `DocumentLink` / backlinks calculés

**Ancrage UC.** [UC-04](conception/usecases/UC-04-gerer-documents-campagne.md), scénario nominal (titre, type optionnel, propriétés structurées, blocs, visibilité, documents liés) ; [UC-03](conception/usecases/UC-03-structurer-scenario.md), scénario nominal (étapes 6-8 — scènes représentées par des documents `SCENE` liés) et A2/A3 (ajout d'éléments liés existants / création d'un élément depuis le scénario).
...
**Ancrage UC.** [UC-04](conception/usecases/UC-04-gerer-documents-campagne.md) A4 — « changement de visibilité », qui renvoie explicitement le détail à UC-08 ; [UC-08](conception/usecases/UC-08-partager-information.md), scénario nominal et arbitrage de granularité.
...
**Ancrage UC.** [UC-13](conception/usecases/UC-13-scenario-reutilisable.md), scénario nominal (« rejouer un scénario depuis la bibliothèque ») et A1 (« marquer un scénario comme réutilisable »).
...
**Ancrage UC.** [UC-05](conception/usecases/UC-05-organiser-dossiers.md) A6 — « dossiers créés à la création d'un espace `CAMPAIGN` ou `ONE_SHOT` », et son complément sur l'espace `PERSONAL`.
...
**Ancrage UC.** [UC-04](conception/usecases/UC-04-gerer-documents-campagne.md), scénario nominal (propriétés structurées, visibilité) ; [UC-14](conception/usecases/UC-14-recherche.md), règles métier (filtrage par visibilité, résultats titre-seul).
...
Ce module couvre cinq use cases : [UC-06](conception/usecases/UC-06-vue-session.md) (vue session), [UC-07](conception/usecases/UC-07-creation-volee-session.md) (création à la volée), [UC-08](conception/usecases/UC-08-partager-information.md) (partage), [UC-09](conception/usecases/UC-09-acces-session-joueur.md) (temps réel joueur) et [UC-12](conception/usecases/UC-12-rejoindre-campagne.md) (vue joueur). Le point 6.d est transverse et renvoie à sa composition complète en §2.3, avec un renvoi complémentaire à §7 pour le transport temps réel.

### 6.1 — UC-06 : machine d'états de session

#### 6.a — Machine d'états de session `LIVE → CLOSED → ARCHIVED`

**Ancrage UC.** [UC-06](conception/usecases/UC-06-vue-session.md), Phase 1 — lancement de la session ; Phase 4 — fermeture ; E3 — accès à une session non `LIVE`.
...
**Ancrage UC.** [UC-07](conception/usecases/UC-07-creation-volee-session.md), scénario nominal et règles métier — création à la volée pendant la session.
...
**Ancrage UC.** [UC-06](conception/usecases/UC-06-vue-session.md), Phase 2 — vue session MJ (tableau de bord configurable).
...
**Ancrage UC.** [UC-08](conception/usecases/UC-08-partager-information.md) (partage — diffusion aux joueurs) ; [UC-09](conception/usecases/UC-09-acces-session-joueur.md) (accès joueur temps réel pendant la session) ; [UC-12](conception/usecases/UC-12-rejoindre-campagne.md) (frontière avec la capacité d'écriture temps réel d'UC-06).
...
**Ancrage UC.** [UC-09](conception/usecases/UC-09-acces-session-joueur.md), règle métier RB-09-01 (fenêtre de grâce de l'accès ponctuel) ; [UC-06](conception/usecases/UC-06-vue-session.md) (accès joueur validé en couche application).
...
**Ancrage UC.** [UC-10](conception/usecases/UC-10-compte-cloud.md) — le cycle de vie complet des opérations sensibles et de la validation email est développé en **§3.a** ; ce paragraphe n'en reprend pas le contenu et se limite à la composition entre le cycle de vie du token et la topologie d'hébergement.
...
**Ancrage UC.** [UC-08](conception/usecases/UC-08-partager-information.md), [UC-09](conception/usecases/UC-09-acces-session-joueur.md), [UC-12](conception/usecases/UC-12-rejoindre-campagne.md) — mêmes ancrages que la diffusion temps réel développée en **§2.3/§6.4**, dont le filtrage par visibilité n'est pas redessiné ici.
...
**Point de vigilance de cohérence corpus — divergence non réconciliée, pas un `[À TRANCHER]` créé ici.** [UC-10](conception/usecases/UC-10-compte-cloud.md) §A4/§E4 décrit la suppression de compte comme **bloquée** tant que le titulaire est propriétaire d'espaces partagés avec des membres actifs (« le système bloque la suppression »), tandis que `identity-access.md` § Invariants métier, invariant 3, marque ce point **« Résolu (ADR-018, W2) »** et affirme au contraire que `User.Delete()` n'est **jamais** bloqué par la possession d'espaces — seul le routage de cascade diffère par type d'espace. Ce dossier ne tranche pas cette divergence entre l'ancrage UC (source de vérité du besoin) et son raffinement domaine ; elle est signalée pour réconciliation, conformément à l'ordre d'autorité posé en tête de ce dossier. Déjà signalée en **§2.2**.

### 8.2 — Renvoi juridique

Les points de conformité RGPD **conçue mais non certifiée** touchant ce dossier — qualification Art. 17 du hard-delete inconditionnel de l'espace `PERSONAL`, instant juridique de référence `deletion_requested_at`, périmètre du DPA Art. 28 pour un contenu personnel décrivant un tiers identifiable, et mise en balance de l'intérêt légitime pour le `display_name` de l'invité — sont consolidés dans [`docs/juridique/cadrage-validation-pre-lancement-eu.md`](juridique/cadrage-validation-pre-lancement-eu.md), qui les recense en 5 axes à valider par un juriste avant tout lancement EU. Ce dossier renvoie à ce cadrage ; il ne re-tranche aucun des axes qui s'y trouvent.
...
| Référence | Rôle dans ce dossier |
|---|---|
| [07-architecture-detaillee.md](architecture/07-architecture-detaillee.md) | Vue transverse par préoccupation technique (persistance, sécurité, RGPD, temps réel, mode local) |
| [06-structure-projets.md](architecture/06-structure-projets.md) | Arborescence de projets, nommage `SharedKernel` (§1.1) |
| [ArchitectureIndex.md](architecture/ArchitectureIndex.md) | Découverte pédagogique, ordre de lecture conseillé du corpus d'architecture |
...
| [docs/juridique/cadrage-validation-pre-lancement-eu.md](juridique/cadrage-validation-pre-lancement-eu.md) | Cadrage RGPD conçue mais non certifiée — 5 axes à valider par un juriste avant tout lancement EU (§8.2) |
