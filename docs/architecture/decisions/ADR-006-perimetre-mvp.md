...
**MVP unique — pas de découpage en deux vagues produit A/B.**
...
**MVP en deux vagues — vague A (vue session solo, local-only) puis vague B (cloud + partage).**
...
- Le trio le plus risqué techniquement (synchronisation local↔cloud, migration, temps réel SignalR) est front-loadé dans le MVP. La migration n'est plus différable — elle devient un livrable du jalon J2 (plan de cadrage Vague 3).
...
- UC-10 étant promu Must, son estimation de complexité (finding H-04 : sous-estimé) doit être revisitée en Vague 3.
