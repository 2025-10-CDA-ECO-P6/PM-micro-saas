# ADR-014 — Modèle d'autorisation API (appartenance ressource↔espace)

> **Annotation 2026-06-12** — Renommage `campaign_id` → `space_id` (généralisation `Campaign` → `Space`, ADR-018). Note ajoutée : un espace `PERSONAL` est mono-membre (`OWNER` = propriétaire) ; l'accès est restreint au propriétaire par construction via P1 (aucun invité, aucun `PLAYER`/`GM`).
...
- Query filters EF Core globaux (soft-delete documents/espaces) → **B5.1**
- Tests IDOR → **B5.2**
...
**Couche Application — appartenance ressource↔espace** (objet de cet ADR)
La couche Application répond à la question : « L'appelant a-t-il accès à l'*espace* de cette ressource ? » C'est le gate antérieur, commun à toutes les ressources d'un espace, indépendant du type de document.
...
Un utilisateur est « membre actif » d'un espace si et seulement si :

```
space_memberships.space_id = <espace cible>
AND space_memberships.user_id = <userId appelant>
AND space_memberships.status = 'ACTIVE'
```

Son rôle (`OWNER` / `GM` / `PLAYER`) est lu depuis `space_memberships.role`. Ce rôle est transmis à `Document.CanBeReadBy` pour résoudre la visibilité.

**Invité (GuestAccess, sans compte)**

Un invité est « actif » sur un espace si et seulement si :

```
guest_accesses.space_id = <espace cible>
...
Toute lecture d'une ressource d'espace compose, en ET (non en OU), les quatre prédicats suivants :
...
| P2 | `spaces.deleted_at IS NULL` ET `spaces.purge_claimed_at IS NULL` | Indépendant de l'appelant | Query filter EF Core global (→ B5.1) |
...
**Cas restauration espace en corbeille** : l'endpoint de restauration est réservé à l'`OWNER` et distinct du chemin de lecture normal. Il vérifie `deleted_at IS NOT NULL` et la borne de restauration définie en B1 (→ ADR-011 pour la valeur) — c'est le seul chemin autorisé à traverser P2. Le chemin de lecture normal ne laisse jamais passer un espace en corbeille.
...
1. **Pipeline behavior MediatR (REST — pull)** : tout `IRequest` portant sur une ressource d'espace implémente `ISpaceScopedRequest` (déclarant son `SpaceId` direct ou résolu). Le behavior intercepte la requête, appelle `IResourceAccessPolicy.CanAccess(...)`, et retourne `403`/`404` avant d'atteindre le handler si la décision est `Denied`. Les handlers ne font pas cette vérification en doublon.

2. **Filtre de diffusion SignalR (push, par message, → B1.7)** : avant de pousser un message vers un abonné, le filtre appelle le **même** `IResourceAccessPolicy.CanAccess(...)`. Il n'y a pas de diffusion indifférenciée par groupe espace. REST et SignalR partagent le même type — non-divergence structurelle garantie.

La résolution transitive du `space_id` (§4) fait partie du contrat d'implémentation de chaque requête / chaque message SignalR.

**Test d'archi CI** (→ **B3.2**) : le test d'architecture doit couvrir deux catégories de handlers :
- **Handlers scopés** (`ISpaceScopedRequest`) : le test vérifie qu'ils passent tous par le pipeline behavior / `IResourceAccessPolicy`. Tout handler scopé qui échapperait au pipeline est un échec CI.
- **Handlers non-scopés** (recherche globale, liste « mes espaces », dashboard — sans `space_id` unique) : ces handlers n'implémentent pas `ISpaceScopedRequest` et filtrent directement sur `space_memberships.user_id = appelant`. Le test doit **catégoriser** les handlers et vérifier que les non-scopés filtrent bien sur l'appartenance membre — sinon angle mort. La liste des handlers non-scopés autorisés est renvoyée à B3.2 pour définition exhaustive.

---

### 4. Matrice ressource → règle d'appartenance

Pour chaque type de ressource accessible par l'API, le chemin de résolution vers `space_id` et les contraintes supplémentaires d'appartenance invité.
...
| `folders` | `folders.space_id` | Oui | Non (pas de navigation bibliothèque hors session) |
| `sessions` | `sessions.space_id` | Oui | Uniquement `guest_accesses.session_id` |
| `invitations` | `invitations.space_id` | Non (lecture MJ uniquement) | Non |
| `guest_accesses` | `guest_accesses.space_id` | Non (gestion MJ uniquement) | Non |
| `space_memberships` | `space_memberships.space_id` | Non (gestion MJ uniquement) | Non |
| `session_view_configs` | `session_view_configs.space_id` | Oui (si session accessible) | Oui (session propre) |
| `document_types` (custom) | `document_types.space_id` | Non (gestion MJ uniquement) | Non |
| `membership_characters` | `membership_characters.space_id` | Non (gestion MJ) | Non (gestion MJ) — lecture : joueur concerné uniquement |

**Ressources avec résolution transitive**

| Ressource | Chemin de résolution | Contrainte supplémentaire |
|---|---|---|
| `document_blocks` | `document_blocks.document_id` → `documents.space_id` | + visibilité domaine du document parent |
| `document_links` | `document_links.source_document_id` → `documents.space_id` **ET** `document_links.target_document_id` → `documents.space_id` | **Appartenance bilatérale + invariant domaine** : un lien cross-espace est **rejeté au niveau domaine** — `Document.LinkDocument(...)` lève un invariant si `source` et `target` n'appartiennent pas au même espace. Il n'existe donc pas de lien dont `source` et `target` seraient dans des espaces différents. La couche Application n'a pas à gérer ce cas : l'invariant le ferme à la source (→ P4 implémentation en **B4**). |
| `document_tags` | `document_tags.document_id` → `documents.space_id` | + visibilité domaine du document parent |
| `session_pinned_documents` | `session_pinned_documents.session_id` → `sessions.space_id` | **Re-applique `Document.CanBeReadBy` via `IResourceAccessPolicy`** — un `GM_ONLY` épinglé ne transite pas vers un joueur/invité |
| `session_live_notes` | `session_live_notes.session_id` → `sessions.space_id` ET `session_live_notes.document_id` → `documents.space_id` | **Re-applique `Document.CanBeReadBy` via `IResourceAccessPolicy`** — un `PLAYER_PRIVATE` (auteur-seul) n'est jamais lisible que par son auteur, sur tous les chemins |
| `session_view_folders` | `session_view_folders.session_view_config_id` → `session_view_configs.space_id` ET `session_view_folders.folder_id` → `folders.space_id` | Les deux chemins doivent pointer vers le même espace |
...
Pendant la fenêtre entre la création de compte et l'activation du `SpaceMembership` par le MJ, le compte nouvellement créé hérite du niveau `GuestAccess` (scope limité de l'accès invité d'origine). Il n'obtient pas automatiquement le niveau `PLAYER`. Le saut de privilège est interdit : l'élévation au rôle `PLAYER` requiert l'activation explicite du `SpaceMembership` par un `OWNER` ou `GM`.
...
2. **Filtre de diffusion SignalR (push, par message, → B1.7)** : avant de pousser un message vers un abonné, le filtre SignalR appelle le **même** `IResourceAccessPolicy.CanAccess(...)`. Il n'y a pas de diffusion indifférenciée par groupe espace.
...
**Conséquence** : un message `GM_ONLY` ne peut jamais être poussé vers un joueur ou un invité, même s'ils sont abonnés au même hub d'espace. Un document `PLAYER_PRIVATE` ne peut jamais être poussé vers un autre appelant que son auteur.
...
Intercepter au niveau du middleware HTTP plutôt que MediatR. Écarté au profit du pipeline behavior : le middleware HTTP n'a pas accès au contexte applicatif résolu (type de ressource, `space_id` résolu transitivement). La résolution transitive nécessite une requête DB contextualisée — ce que MediatR fournit proprement dans la couche Application.

**Autorisation basée sur les rôles (RBAC) seul**
RBAC vérifie le rôle de l'utilisateur mais pas l'appartenance à l'espace spécifique. Un `PLAYER` d'un espace A pourrait lire une ressource de l'espace B s'il en connaît l'identifiant. Rejeté : la contrainte d'appartenance ressource↔espace est précisément ce que RBAC seul ne couvre pas.
...
|---|---|
| Appelant non-membre → lecture d'une ressource d'un autre espace | 404 (ne révèle pas l'existence) ou 403 |
| Invité scope `SESSION` → ressource hors de sa session | 403 |
| Lecture d'un espace avec `deleted_at IS NOT NULL` | Invisible (filtre P2) |
| Lecture d'un espace avec `purge_claimed_at IS NOT NULL` | Invisible (filtre P2) |
...
- Chaque `IRequest` portant sur une ressource d'espace déclare son `SpaceId` résolu (interface `ISpaceScopedRequest` ou équivalent).
...
- La résolution transitive du `space_id` (ex. `document_blocks` → `documents`) fait partie du contrat de chaque requête / message et peut nécessiter une pré-requête légère ou une jointure.

### Test d'archi (→ B3.2)

Un test ArchUnit (ou équivalent .NET) catégorise les handlers en deux familles et vérifie :
- **Handlers scopés** (`ISpaceScopedRequest`) : tous passent par le pipeline behavior / `IResourceAccessPolicy`. Tout ajout non conforme échoue en CI.
- **Handlers non-scopés** (liste exhaustive définie en B3.2 — ex. recherche globale, liste « mes espaces ») : chacun filtre sur `space_memberships.user_id = appelant`. Tout handler non-scopé qui n'appliquerait pas ce filtre est un échec CI.
...
L'invariant d'appartenance garantit qu'un utilisateur ne peut pas accéder aux données d'un espace auquel il n'appartient pas — ce qui inclut les données à caractère personnel portées par les documents, profils de personnages et `LIVE_NOTE`. Cet invariant contribue à l'obligation de minimisation et de contrôle d'accès (Art. 5(1)(f) RGPD).
...
- **B5.1** — Configuration des query filters EF Core globaux pour P2 (`spaces.deleted_at`, `spaces.purge_claimed_at`) et P3 (`documents.is_deleted`).
