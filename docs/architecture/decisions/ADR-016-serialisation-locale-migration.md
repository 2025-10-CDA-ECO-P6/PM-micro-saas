...
- `spaces` (racine, un ou plusieurs — incluant l'espace `PERSONAL`)
- `folders` (arborescence rattachée à l'espace)
...
- `document_types` custom uniquement (types créés par l'utilisateur dans l'espace ; les types système sont seedés côté serveur et non transportés)
...
**Espace `PERSONAL`** : en mode local, le contenu capturé hors campagne vit dans l'espace personnel. Cet espace est inclus dans la racine `spaces` au même titre que les espaces de type CAMPAIGN. Il n'a pas de sessions ni de membres (`sessions`, `session_pinned_documents`, `session_live_notes` et les memberships sont absents de sa représentation sérialisée) — les exclusions listées ci-dessous s'appliquent identiquement. À l'import, l'espace `PERSONAL` est rattaché au compte créé (`owner_id` assigné, cohérent §2.1 — la propriété naît à l'import).

**Exclusions explicites** :
- Sessions en cours (`status = LIVE`) : une session active ne peut pas être migrée ; elle doit être clôturée avant export. **Correction — Annotation du 2026-06-10** : l'exclusion antérieure reposait sur la prémisse « le mode local UC-01 ne comprend pas de session — ces entités n'existent pas dans le store local ». Cette prémisse est factuellement fausse : UC-06 et UC-07 spécifient que la vue session du MJ est disponible en mode local sans compte, et que des sessions terminées, notes de session, épinglages et résumés y persistent. L'exclusion est inversée le 2026-06-10 — l'historique de session fait partie du périmètre migré avec les données d'espace. Restent explicitement exclues : (i) les sessions en statut `LIVE` (elles ne sont pas migrables en l'état ; voir §3), (ii) `session_view_configs` (préférence d'affichage, recréée à l'import comme à la création d'espace), (iii) `session_view_folders` (contenu de la configuration de vue — même traitement).
- `users` et memberships (`space_memberships`, `membership_characters`) : absents du mode local sans compte.
- Tout flag de cycle de vie serveur (`deleted_at` sur espaces, `purge_claimed_at`, `is_deleted` sur documents) : ces états sont gouvernés par le serveur et ne doivent pas être transportés.
...
- `document.slug` : revalidé (unicité par espace et type, format) ; en cas de collision de slug, l'item est rejeté avec la raison correspondante dans le rapport (§3).
- `document_type.slug` (pour les types custom) : résolu et validé comme appartenant à l'espace créé à l'import (voir §2 — frontière de confiance).
- `session.status` : revalidé contre l'énumération `CLOSED | ARCHIVED` ; une session en statut `LIVE` dans le payload entraîne le rejet de l'espace concerné (raison `LIVE_SESSION_IN_PAYLOAD` dans le rapport, cohérent avec §3). Le titre et le résumé de session sont traités comme contenu libre, sanitisés comme le contenu des blocs (§2.4).
...
Le mode local n'a pas de compte. La migration n'est pas une « preuve d'appartenance » — c'est une **assignation de propriété** : l'utilisateur qui migre devient propriétaire (`owner_id`) des données importées au moment de l'import (ADR-014 — `owner_id` naît à l'import). Il n'y a rien à prouver, parce qu'il n'y avait pas de propriétaire avant. Cela vaut pour tous les espaces migrés, y compris l'espace `PERSONAL` : à l'import, il est rattaché au compte créé via ce même mécanisme d'assignation.
...
#### 3.1 Stratégie transactionnelle par espace

La granularité de la transaction d'import est **l'espace**. Le serveur ne fait pas de tout-ou-rien sur le payload entier (tous les espaces ou aucun), ni document par document. L'espace est l'unité cohérente : un espace et toutes ses entités dépendantes — documents, sessions, références d'épinglage et notes rattachées — sont importés en transaction ou rejetés en bloc. Cette règle s'applique à tous les espaces du payload, y compris l'espace `PERSONAL`.

**Séquence** :

1. Le serveur effectue un **dry-run de pré-validation** sur chaque espace du payload : il applique les Value Objects sur tous les champs gouvernés (incluant `session.status`) et collecte les rejets sans écrire en base.
2. Il produit un **rapport de rejets** indiquant, pour chaque espace invalide, les raisons du rejet (ex. `SLUG_COLLISION`, `INVALID_PROPERTIES`, `UNKNOWN_DOCUMENT_TYPE_SLUG`, `UNSUPPORTED_SCHEMA_VERSION`, `LIVE_SESSION_IN_PAYLOAD`).
3. Les espaces valides sont importés dans des transactions séparées. Les espaces invalides ne sont pas importés.
4. Les données locales correspondant aux espaces rejetés restent **intactes dans le navigateur** — conformément à l'acquis E5 d'UC-10 : aucune donnée locale n'est supprimée avant confirmation explicite de la migration côté serveur.
...
- Les `sessions` sont créées après tous les `documents` de l'espace.
...
En cas de **références orphelines** dans le payload — un document référençant un dossier parent dont l'espace a été rejeté, ou un document référençant une cible de lien absente du payload — le comportement est le suivant : l'entité orpheline est rejetée de manière cohérente avec la sous-arborescence concernée, et le rejet est consigné dans le rapport.

Les **arêtes de cycle nullable** dans le payload (ex. `folders.default_template_document_id`) sont traitées en deux passes à l'import, par analogie avec la passe 1 de la saga `SpaceDeleted` d'ADR-011 : les références cycliques sont d'abord résolues à NULL, puis rétablies après la création des entités cibles.

#### 3.3 Idempotence

Chaque opération de migration est associée à un **`migration_batch_id`** (UUID v4 généré côté client au déclenchement de la migration). Le serveur **valide le format UUID v4** du `migration_batch_id` et **rejette la requête si le format est invalide** (raison `INVALID_MIGRATION_BATCH_ID`). Le serveur utilise cet identifiant pour garantir l'idempotence : un espace déjà importé dans le cadre d'un même `migration_batch_id` n'est pas importé une seconde fois. L'idempotence est **scopée par utilisateur authentifié** — deux utilisateurs différents peuvent soumettre le même `migration_batch_id` sans collision. Ce mécanisme couvre le cas E5 de UC-10 (migration en attente, reprise sur réseau instable) sans créer de doublons.
...
Lorsqu'une migration est déclenchée sur un compte pour lequel le navigateur contient des données locales, le serveur ou le client **présente à l'utilisateur les espaces détectés** (titres des espaces locaux — y compris l'espace personnel —, volume estimé, date d'export), y compris l'historique de session de chaque espace (sessions terminées, notes de session, documents épinglés, résumés), et requiert une **confirmation explicite** avant de lancer l'import.

Cette confirmation joue le rôle de **reconnaissance des données** : l'utilisateur voit ce qui va être importé — incluant l'historique de session et l'espace personnel — et peut identifier si ces données lui appartiennent ou correspondent à des données d'un autre utilisateur du même poste. Sur un poste partagé, ce gate borne l'appropriation silencieuse : un utilisateur qui déclenche par inadvertance le flux de migration voit les espaces détectés et peut interrompre avant import. Le gate signale aussi toute session en cours (statut `LIVE`) qui devra être clôturée avant migration — elle n'est pas importée, mais l'utilisateur en est informé.
...
Rejeter l'ensemble du payload si un seul espace est invalide. Écarté : une erreur de slug dans un espace ne doit pas bloquer l'import d'espaces entièrement valides. La granularité par espace offre la récupérabilité maximale sans sacrifier la cohérence.

**Document par document**
Importer les documents valides et rejeter les documents invalides au sein d'un même espace. Écarté : la cohérence d'un espace dépend de l'intégralité de ses entités (dossiers, types, documents, liens). Un import partiel produirait un espace incohérent côté serveur (document sans dossier, lien orphelin). L'espace est la bonne unité transactionnelle.
...
ADR-001 §Compléments pose l'invariant : le mode local peut être plus permissif que le serveur, mais ne doit pas accepter en écriture ce que le serveur refuse. Cet invariant est une **discipline de conception dirigée**, pas une propriété vérifiée par outillage.
...
| **ADR-011** | Ordre topologique — la passe 1 de déliaison des arêtes de cycle de la saga `SpaceDeleted` est le miroir de la résolution des cycles à l'import (§3.2). |
| **ADR-013** | Données invité — si un espace rejeté contient des données marquées comme appartenant à un tiers (F-13 / données invité), le rejet de l'espace protège ces données ; renvoi léger vers ADR-013 pour la politique de traitement de ces cas. |
...
| **ADR-018** | Généralisation espace — renommage `campaign_id`→`space_id`, racine `campaigns`→`spaces`, saga `CampaignDeleted`→`SpaceDeleted`. L'espace `PERSONAL` est inclus dans le périmètre sérialisé (§1.2) et soumis à la transactionnalité par espace (§3.1). |
