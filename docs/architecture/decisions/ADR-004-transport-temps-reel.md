...
Le partage temps réel du MJ vers les joueurs est une fonctionnalité promise dans la conception (UC-08/09) et présentée comme un différenciateur central du produit. Aucune technologie de transport n'avait été choisie, alors que ce choix conditionne l'architecture d'infrastructure (connexions persistantes, sticky sessions), le modèle d'autorisation du canal invité, et le coût d'exploitation.

---

## Décision

**SignalR dès le départ.** SignalR est natif à ASP.NET Core, gère automatiquement les fallbacks (WebSocket → Server-Sent Events → long-polling), et est isolé dans le module `Haversack.Infrastructure.Notifications`.
...
Cette option était moins coûteuse en infrastructure (pas de connexions persistantes, pas de sticky sessions) et aurait été adéquate pour un besoin de latence de quelques secondes, tolérable dans le contexte d'une session de jeu de rôle. Cette option viable a été écartée au profit d'une expérience push native.
...
- L'authentification du canal invité sans compte (token `GuestAccess` porté sur le canal SignalR) est à concevoir, avec propagation des révocations et expirations en cours de session active (J3).
- L'empreinte des sessions longues (sessions de jeu de plusieurs heures) sur les connexions persistantes est un point de surveillance opérationnelle.
- Le module `Haversack.Infrastructure.Notifications` est isolé dès J0 (structure décidée dans ADR-008).
