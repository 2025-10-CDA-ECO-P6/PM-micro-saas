...
- Les diagrammes de classes du Core et les trois fichiers où `CharacterId`, `ScenarioId`, `SceneId` subsistent doivent être nettoyés et alignés sur `DocumentId` (J0).
...
- La recherche full-text sur `properties` n'est pas indexée au MVP — la recherche reste titre-seul. Cette limite est assumée.
...
- La conception du VO `DocumentProperties` et des schémas seedés est un livrable de la phase de conception.
...
- **Promotion en colonnes.** Les champs `characterId` et `guestAccessId` des `LIVE_NOTE` passent de `documents.properties` (jsonb) à des colonnes nullable indexées de premier niveau sur la table `documents`. Ce changement débloque l'invariant d'autorisation décrit dans ADR-007.
