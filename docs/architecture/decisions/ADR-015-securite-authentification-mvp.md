...
#### 2.2 Parcours Google OAuth (email réputé vérifié)

Lors d'une connexion ou création de compte via Google, le claim `email_verified` est lu depuis le token d'identité Google (ID token). Si `email_verified = true`, l'email est réputé vérifié par l'IdP : Haversack n'envoie pas de second email de validation, ce qui est conforme à la posture « friction nulle ».

**Dépendance externe — maillon NON VÉRIFIABLE IN BUILD** : la confiance dans le claim `email_verified` de Google est une dépendance au comportement de l'IdP. Le gate applicatif (§2.3 ci-dessous) est testable en CI via un provider OAuth mocké. En revanche, la fiabilité réelle du claim Google — le fait que Google ne produise pas de claim `email_verified = true` pour un email non vérifié — dépend du provider et ne peut pas être garantie par les tests applicatifs Haversack.

#### 2.3 Liaison OAuth à un compte préexistant

La liaison d'un compte OAuth (Google) à un compte email/mot de passe **préexistant** est autorisée uniquement si l'email du compte préexistant est **prouvé vérifié** au sens du §2.1. La liaison est interdite si l'email du compte préexistant n'a pas été validé.

**Motif** : un email non vérifié est un email que n'importe qui peut avoir renseigné (spam d'inscription, usurpation). Autoriser la liaison OAuth vers ce compte revient à permettre à un tiers — disposant d'un compte Google sur cet email — de prendre le contrôle du compte Haversack sans que l'usurpateur ait prouvé sa maîtrise de l'email. Ce vecteur est documenté sous le nom « pre-account-hijacking » dans la littérature OAuth.
...
La fiabilité du claim `email_verified` de Google (§2.2) est une dépendance externe non vérifiable par les tests applicatifs Haversack. Le gate applicatif côté Haversack (§2.3 — interdiction de liaison OAuth vers un compte à email non vérifié) est testable en CI via provider OAuth mocké.

---

## Points à trancher

- **B1.5** — Implémentation des quatre volets (configuration `PasswordOptions` + `IPasswordHasher`, validation email + `IEmailVerificationPolicy`, cycle de vie des tokens + `ITokenDenylist` + `ITokenValidator`, rate limiting). Valeurs exactes des seuils de rate limiting et TTL du token de reset à arrêter à cette occasion.
