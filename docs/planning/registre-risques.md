...
| 10 | D-01 | Double référent de nomenclature de jalon et ancrages non littéraux `[À RATIFIER OPÉRATEUR]` | Coût & délai | Élevée | Moyenne |
...
**D-01 — Double référent de nomenclature de jalon et ancrages non littéraux** `[À RATIFIER OPÉRATEUR]`
*Type : risque.*
- **Description** : le corpus porte deux usages distincts d'un même symbole de jalon selon la décision d'architecture consultée — l'une l'assimile au jalon local, l'autre à une préoccupation cloud. La roadmap d'entrée en build documente elle-même ce double référent et propose, sans l'imposer, un axe de lecture canonique (jalon = local-only), avec obligation de préciser le sens retenu tant que le point n'est pas ratifié.
- **Probabilité** : Quasi-certaine — incohérence de nomenclature déjà présente dans le corpus.
- **Impact** : Modéré — dimension dominante : coût & délai (erreur d'ordonnancement si le sens retenu n'est pas précisé à chaque usage futur) ; borné par la proposition d'axe canonique.
- **Criticité inhérente** : Élevée.
- **Criticité résiduelle cible** : Moyenne.
- **Traitement** : Réduire.
- **Mitigation** : un axe canonique est proposé par la roadmap d'entrée en build, avec obligation de préciser le sens retenu tant que le point n'est pas ratifié.
- **Propriétaire** : Direction de projet.
- **Échéance** : socle.
- **Confiance** : sous réserve — l'axe canonique reste à ratifier par l'opérateur.
- **Source** : roadmap d'entrée en build.
- **Recoupements** : la roadmap documente ce constat directement ; ce registre le reprend comme point de pilotage sans y ajouter d'élément.
