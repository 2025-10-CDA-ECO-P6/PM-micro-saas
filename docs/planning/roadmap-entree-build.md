...
> **Statut** : artefact de planification dédié, référencé par [CdC §11](../cahier-des-charges.md#11-phasage--jalons) et [ADR-011 §Points à trancher](../architecture/decisions/ADR-011-cascade-integrite-referentielle.md) comme le document produit « en amont de l'entrée en construction ». Audience : équipe de build (développeur·se solo ou équipe restreinte prenant en charge l'implémentation) et opérateur (arbitrages de gate).
...
- **ni le MoSCoW** ([CdC §3](../cahier-des-charges.md#3-périmètre-moscow) — Must/Should/Could/Won't, [ADR-006](../architecture/decisions/ADR-006-perimetre-mvp.md) pour la trace des arbitrages fusionnés) ;
- **ni le phasage macro** ([CdC §11](../cahier-des-charges.md#11-phasage--jalons) — nomenclature Vague 0/1/2/3, indicative et non figée).
...
Ce que ce document **fait** : il prend la séquence de jalons déjà nommée dans [ADR-006 §Conséquences](../architecture/decisions/ADR-006-perimetre-mvp.md) (J1 → J2 → J3) et le préalable J0 documenté dans [ADR-008 §Compléments post-revue](../architecture/decisions/ADR-008-structure-solution.md), et les assemble en une séquence unique typée, avec pour chaque jalon : les préalables bloquants déjà actés, les critères de sortie factuels déjà déductibles du corpus, l'ordre interne C#-first déjà acté par [ADR-001 §Compléments](../architecture/decisions/ADR-001-execution-domaine-mode-local.md) et [ADR-008 §Compléments](../architecture/decisions/ADR-008-structure-solution.md), et le rattachement des livrables/tests nommés par chaque ADR à leur jalon. Il propose enfin — sans trancher — une réconciliation de la double nomenclature `J` signalée comme incohérence ouverte par [CdC §11](../cahier-des-charges.md#11-phasage--jalons).

**En cas de conflit entre ce document et un ADR, une US ou le CdC** : la source citée fait foi ; ce document est un artefact d'ordonnancement, pas une nouvelle autorité de corpus (même posture que [`cadrage-validation-pre-lancement-eu.md`](../juridique/cadrage-validation-pre-lancement-eu.md) vis-à-vis des ADR RGPD).
...
J1 (local-only)
   │
[GATE MARCHÉ]  ── NON-VERIFIABLE-IN-BUILD ── gate humain go/no-go
   │
J2 (cloud + migration)
   │
[GATE JURISTE EU]  ── NON-VERIFIABLE-IN-BUILD ── gate humain go/no-go, avant lancement (pas avant build)
   │
J3 (partage + temps réel)
```

**Note de lecture du diagramme** : la chaîne à flèche unique ci-dessus reflète l'ordre de citation, pas une dépendance de blocage entre `[GATE JURISTE EU]` et J3. Le gate juriste bloque le **lancement/release EU** (mise en production commerciale) — il ne bloque **pas le build de J3**, qui peut être construit en parallèle de l'instruction juridique (voir §3.5, « Peut être instruit en parallèle du build »). Seul `[GATE MARCHÉ]`, en amont, conditionne réellement l'entrée dans le jalon suivant (J2).
...
| **(b) Gate humain go/no-go** | `[GATE MARCHÉ]`, `[GATE JURISTE EU]` | **`NON-VERIFIABLE-IN-BUILD`** — aucun test automatisé, aucun linter, aucune build Haversack ne peut trancher ces deux gates ; la dépendance est externe (télémétrie interprétée par l'opérateur pour le premier, avis juriste pour le second) |
...
- J1 → J2 → J3, enchaînement sans découplage : [ADR-006 §Conséquences](../architecture/decisions/ADR-006-perimetre-mvp.md) l.46 (« Les jalons de build se compressent : J1 (local-only), J2 (cloud + migration), J3 (partage + temps réel) doivent être enchaînés sans découplage produit entre eux »).
- J0 : préalable de structure non nommé « J0 » explicitement dans ADR-006, mais ses livrables (squelette Domain/Application, test d'archi CI) sont documentés comme antérieurs à tout jalon applicatif dans [ADR-008 §Compléments post-revue](../architecture/decisions/ADR-008-structure-solution.md) l.56-60. Cette roadmap le nomme J0 pour compléter la séquence — voir §5 pour la discussion de nomenclature.
- `[GATE MARCHÉ]` : [ADR-006 §Compléments post-revue](../architecture/decisions/ADR-006-perimetre-mvp.md) l.67 (« Un gate de validation marché doit être posé avant d'engager le build lourd cloud + temps réel »).
- `[GATE JURISTE EU]` : [`cadrage-validation-pre-lancement-eu.md`](../juridique/cadrage-validation-pre-lancement-eu.md) (5 axes, tous « à valider juriste », gate humain hors CI) ; positionné **avant lancement EU**, distinct d'un jalon de build — voir §3.5 sur cette distinction.
...
| **[GATE MARCHÉ]** | Go/no-go opérateur sur télémétrie J1 | J1 stabilisé | Décision opérateur tracée |
| **J2** | Cloud + migration : EF Core, auth, handler d'import, cascade RGPD | J1 stable + gate marché = go | Migration d'un jeu de données J1 réussie de bout en bout |
| **[GATE JURISTE EU]** | Go/no-go avis juriste, 5 axes RGPD | Peut être instruit en parallèle de J2/J3 ; bloque le **lancement EU**, pas le build | 5 axes statués par un juriste |
...
- Confirmation à l'entrée en build des **8 ADR de nature pré-implémentation ou mixte** : ADR-003 (stack front), ADR-004 (transport temps réel), ADR-008 (structure solution), ADR-011 (cascade RGPD), ADR-015 (sécurité authentification), ADR-016 (sérialisation locale/migration), ADR-017 (modèle IndexedDB), ADR-018 (généralisation Space). Chacun porte l'annotation *« décision d'architecture actée en phase conception, à confirmer à l'entrée en build »* — la confirmation est un acte de build, pas de conception (renvoi : `docs/BACKLOG.md` § Préalables & jalons de build, item 1).
...
- Le squelette Domain/Application compile et respecte la structure décrite dans `06-structure-projets.md`.
...
### 3.3 [GATE MARCHÉ] — go/no-go avant le build lourd cloud + temps réel
...
- **Dépendance d'entrée** : J1 stable **et** `[GATE MARCHÉ]` = go. Les deux conditions sont cumulatives, pas alternatives ([ADR-006 §Conséquences](../architecture/decisions/ADR-006-perimetre-mvp.md)).
...
**Préalable bloquant rattaché** : J1 stable + `[GATE MARCHÉ]` = go (cumulatif) ; **promotion ADR-002 des colonnes `characterId`/`guestAccessId` (`LIVE_NOTE`)**, préalable schéma qui débloque l'invariant d'autorisation ADR-007 (voir ci-dessus) ; F-02/F-06/F-10/F-11 (auth) déjà reclassés bloquants MVP par [ADR-007 §Compléments](../architecture/decisions/ADR-007-rgpd-autorisation-api.md) ; F-04 (base légale invités, [ADR-013](../architecture/decisions/ADR-013-rgpd-donnees-invites.md)) — bloquant **avant tout lancement EU**, donc au plus tard au `[GATE JURISTE EU]`, pas nécessairement avant le début technique de J2.

---

### 3.5 [GATE JURISTE EU] — validation juridique pré-lancement

**Nature** : `NON-VERIFIABLE-IN-BUILD`. Bloque le **lancement EU** (mise en production commerciale), pas le build technique — distinction explicite à ne pas confondre avec un jalon de build ([`cadrage-validation-pre-lancement-eu.md` § Gate humain hors CI](../juridique/cadrage-validation-pre-lancement-eu.md)).

**Contenu — 5 axes, tous « à valider juriste »** (source unique de détail : [`docs/juridique/cadrage-validation-pre-lancement-eu.md`](../juridique/cadrage-validation-pre-lancement-eu.md), ce document n'en reproduit que la synthèse) :
...
- **J3** : Application/Domain (événements `GuestAccessRevoked`/`Expired` déjà modélisés) → Infrastructure.Notifications (SignalR, isolé dès J0 selon [ADR-004 §Conséquences](../architecture/decisions/ADR-004-transport-temps-reel.md)) → Angular (vue session temps réel).

---

## 5. Réconciliation de nomenclature — `[À RATIFIER OPÉRATEUR]`

### 5.1 Le double référent constaté

Le corpus porte deux usages distincts du symbole « J1 » :
...
- **ADR-007** ([§Conséquences](../architecture/decisions/ADR-007-rgpd-autorisation-api.md) l.48) : « le jalon J1 (Vague 3) » — appliqué à l'intégration de l'invariant d'autorisation API, une préoccupation **cloud**.

[CdC §11](../cahier-des-charges.md#11-phasage--jalons) constate lui-même cette « incohérence apparente » et la laisse ouverte, renvoyée explicitement à « l'artefact de planification dédié » — c'est-à-dire au présent document.

### 5.2 Axe canonique retenu par cette roadmap

Cette roadmap adopte **J0/J1/J2/J3 comme axe canonique**, avec **J1 = local-only** (au sens ADR-006), pour la raison suivante : c'est l'usage le plus densément documenté (ADR-001, ADR-006, ADR-016, ADR-017 s'y réfèrent tous de façon cohérente), et c'est celui qui structure la dépendance technique explicite « pas d'EF cloud en J1 » (§1, §4). La mention isolée d'ADR-007 (« J1 (Vague 3) ») est traitée ci-dessous comme une **divergence de corpus à signaler**, pas comme une variante à absorber silencieusement.

**Ce choix est une convention d'ordonnancement de cette roadmap, pas une correction d'ADR** : ADR-007 n'est pas modifié par ce document (hors périmètre — bandeau de cadrage). Le point reste `[À RATIFIER OPÉRATEUR]` — voir §5.4.

### 5.3 Proposition de mapping J ↔ Vague (CdC §11) ↔ codes de renvoi

Le tableau suivant est une **proposition**, pas une décision. Il croise l'axe canonique de cette roadmap (J0-J3) avec la nomenclature Vague de CdC §11 et les codes de renvoi les plus fréquemment cités par les ADR (P0.5, B1.x, M1, L1, P6, P7, B3.2).

| Jalon (axe canonique) | Vague CdC §11 | Codes de renvoi rattachés | Nature de l'ancrage |
|---|---|---|---|
| *(préalable, avant J0)* | Vague 0 | P0.5 | **Inféré, à confirmer** — CdC §11 ne rapproche P0.5 de Vague 0 que par antériorité déduite (« hors périmètre du lot P0.5 » implique antérieur à Vague 1), pas par ancrage littéral. |
| J0 | (chevauche Vague 0 → Vague 1) | — | Non ancré littéralement ; cette roadmap le nomme J0 pour compléter la séquence macro (§1), sans correspondance Vague explicite dans le corpus actuel. |
| J1 (local-only) | proche de Vague 1 en contenu (M1, L1) | **M1** (ADR-016/017, format sérialisation + IndexedDB + sécurité mode local), **L1** (sous-lot de M1, ADR-017 — schéma IndexedDB détaillé), **P6** (implémentation Angular des services IndexedDB) | Ancrage par **contenu** (les codes M1/L1/P6 portent tous sur le mode local), pas par citation explicite d'une Vague dans les ADR sources. |
| J1 → J2 (bascule migration) | frontière Vague 1/2 | **P7** (handler d'import serveur, tests e2e multi-versions IndexedDB) | P7 porte à la fois sur du code serveur (J2) et sur la validation du contrat local (J1) — code de frontière, pas assignable à un seul jalon. |
| J2 (cloud + migration) | **Vague 1 selon l'ancrage documenté** (« Vague 1 ⊇ lot B1.x », [ADR-011 l.271](../architecture/decisions/ADR-011-cascade-integrite-referentielle.md)) | **B1.x** (B1.4 RGPD étendu, B1.5 auth + `ReclaimViaFederatedProof`, B1.6 données invité espace vivant, B1.7 runtime SignalR, B1.9 chaîne média, B1.10 contrat OpenAPI), **B3.2** (test archi CI, définition exhaustive), **B5.1/B5.2** (query filters, tests IDOR) | **Tension à signaler** : l'ancrage documenté « Vague 1 ⊇ B1.x » situe B1.x en Vague 1 — mais le **contenu** de la plupart des B1.x (auth cloud, migration, cascade RGPD étendue) correspond, sur l'axe canonique de cette roadmap, à **J2** plutôt qu'à J1. C'est exactement le type de discordance que §5.1 signale : un même code de renvoi (Vague 1) recouvre, selon qu'on le lit par contenu ou par citation documentée, deux jalons différents de l'axe J. |
| J3 (partage + temps réel) | Vague 2 (indicatif, non ancré) | **B1.7** (runtime SignalR, également listé J2 — recoupement), **B8.2** (tests anti-fuite SignalR) | Non ancré littéralement à une Vague dans le corpus actuel — rattachement par contenu (temps réel) uniquement. |
| Lancement EU (`[GATE JURISTE EU]`) | hors séquence Vague (jalon transverse) | Lot 14 (juridique, `docs/BACKLOG.md`) | Le cadrage juridique n'est pas rattaché à une Vague de build — il conditionne le lancement, pas une Vague technique. |

**Ce que ce tableau révèle** : la tension sur la ligne J2/B1.x n'est pas un artefact de cette roadmap — elle est la **preuve** que la double lecture « Vague 1 » (par citation documentée vs par contenu) reproduit, à l'échelle des codes de renvoi, la même ambiguïté que le double référent « J1 » entre ADR-006 et ADR-007. Résoudre l'un sans résoudre l'autre laisserait une incohérence résiduelle.

### 5.4 Statut de cette section

**`[À RATIFIER OPÉRATEUR]`** — cette roadmap propose l'axe canonique (§5.2) et le mapping (§5.3) ; elle ne les impose pas comme nouvelle autorité de corpus. La qualification définitive (renommer ADR-007:48, ajuster CdC §11, ou documenter la coexistence des deux lectures) reste à trancher par l'opérateur. Tant que ce point n'est pas ratifié, toute référence future à « J1 » dans un artefact de build doit préciser explicitement le sens retenu (local-only vs cloud/Vague 3) pour éviter une ambiguïté silencieuse.
...
- **Assets d'identité** (logo, charte graphique/typo/grille, ressources icônes/composants) — **HAND-OFF présentation**. Mentionné pour visibilité de trajectoire ; le détail (production, validation) relève de `docs/conception/presentation/**`, non modifié par ce document (renvoi : `docs/BACKLOG.md` § Préalables & jalons de build, dernier item).
...
- [`docs/cahier-des-charges.md` §11](../cahier-des-charges.md#11-phasage--jalons) — macro-ordonnancement de référence (Vague 0/1/2/3), incohérence de nomenclature signalée.
- [`docs/BACKLOG.md`](../BACKLOG.md) § Préalables & jalons de build — liste consolidée des 8 items non réalisables dans le repo documentaire.
- [`docs/juridique/cadrage-validation-pre-lancement-eu.md`](../juridique/cadrage-validation-pre-lancement-eu.md) — détail des 5 axes du `[GATE JURISTE EU]`.
...
- [`docs/architecture/ArchitectureIndex.md`](../architecture/ArchitectureIndex.md) et [`docs/architecture/07-architecture-detaillee.md`](../architecture/07-architecture-detaillee.md) — vue transverse par préoccupation, non redondante avec l'ordonnancement temporel de ce document.
