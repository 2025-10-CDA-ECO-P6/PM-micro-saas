...
- **Tranché** (décision d'entrée en build du 2026-09-03) : les trois règles ne se vérifient pas au même niveau, et **les deux mécanismes que la source présentait en alternative sont retenus, chacun sur les règles qu'il vérifie honnêtement**.
  - **Règle 2 — NetArchTest.** « Chaque contexte respecte les frontières définies par les namespaces » porte sur des dépendances **entre types, à l'intérieur d'un même projet** : ADR-008 fait des quatre bounded contexts des namespaces d'un projet unique, non des assemblies séparées. Il n'y a donc aucune référence de projet à observer. NetArchTest travaille sur l'assemblage compilé et voit la dépendance quelle que soit sa forme syntaxique — nom pleinement qualifié, `global using`, alias, générique, signature de méthode. Un contrôle sur le texte source raterait ces formes **en silence**, ce qui est le pire défaut pour un contrôle censé remplacer une discipline de revue.
  - **Règles 1 et 3 — contrôle du graphe des références de projet.** « Le Domaine n'importe aucune assembly d'Infrastructure ni de Présentation » et « Application et Infrastructure ne dépendent que du Domaine » portent sur des **références entre projets**, lisibles dans les fichiers de projet sans aucune ambiguïté de syntaxe. Un contrôle dédié les vérifie **avant la compilation**, donc échoue plus tôt, et n'ajoute aucune dépendance.

  *Ce que chacun ne fait pas.* NetArchTest ne voit que des types : ni convention de nommage, ni arborescence, ni chaîne interdite. Le contrôle des références de projet ne voit rien de ce qui se passe à l'intérieur d'un projet. Les deux sont complémentaires, non redondants.
