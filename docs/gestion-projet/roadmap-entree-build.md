...
- `[GATE JURISTE EU]` : [`cadrage-validation-pre-lancement-eu.md`](../securite/conformite/cadrage-validation-pre-lancement-eu.md) (5 axes, tous « à valider juriste », gate humain hors CI) ; positionné **avant lancement EU**, distinct d'un jalon de build — voir §3.5 sur cette distinction.
...
- Confirmation à l'entrée en build des **8 ADR de nature pré-implémentation ou mixte** : ADR-003 (stack front), ADR-004 (transport temps réel), ADR-008 (structure solution), ADR-011 (cascade RGPD), ADR-015 (sécurité authentification), ADR-016 (sérialisation locale/migration), ADR-017 (modèle IndexedDB), ADR-018 (généralisation Space). Chacun porte l'annotation *« décision d'architecture actée en phase conception, à confirmer à l'entrée en build »* — la confirmation est un acte de build, pas de conception.
...
**Nature** : `NON-VERIFIABLE-IN-BUILD`. Bloque le **lancement EU** (mise en production commerciale), pas le build technique — distinction explicite à ne pas confondre avec un jalon de build ([`cadrage-validation-pre-lancement-eu.md` § Gate humain hors CI](../securite/conformite/cadrage-validation-pre-lancement-eu.md)).
...
L'axe **J0/J1/J2/J3**, avec **J1 = local-only** (au sens ADR-006), est acté comme **axe canonique unique de phasage pour l'ensemble du corpus** — ratifié par l'opérateur le **2026-09-01**. Raison retenue : c'est l'usage le plus densément documenté (ADR-001, ADR-006, ADR-016, ADR-017 s'y réfèrent tous de façon cohérente), et c'est celui qui structure la dépendance technique explicite « pas d'EF cloud en J1 » (§1, §4).
...
| Lancement EU (`[GATE JURISTE EU]`) | cadrage juridique interne (cf. [`cadrage-validation-pre-lancement-eu.md`](../securite/conformite/cadrage-validation-pre-lancement-eu.md)) | Hors séquence de jalons techniques — le cadrage juridique conditionne le lancement, pas un jalon de build. |
