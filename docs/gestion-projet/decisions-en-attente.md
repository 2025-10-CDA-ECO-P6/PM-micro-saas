...
**Ce que ça bloque.** Aujourd'hui, aucune tâche de `docs/gestion-projet/plan-de-travail.md` ne porte le marqueur `TROU` (`tools/VerifMailleAgregat` le confirme) : `plan-de-travail.md § 2` nomme déjà les modules de la couche cliente par la **préoccupation** que le corpus leur donne ailleurs, plutôt que par un chemin de fichier — ce qui contourne le manque pour les tâches existantes. `TROU` reste défini (`plan-de-travail.md § 2`) pour une tranche future qui viserait un module de code que le corpus ne nomme pas encore ; une telle tranche ne serait pas protégée par le champ `En conflit avec`, qui compare des modules déclarés.

**Ce qu'il faut savoir pour répondre.** `plan-de-travail.md § 2` énonce que la maille par agrégat **ne dépend pas** de cette décision : un agrégat est nommé et stable indépendamment de sa future disposition en fichiers, et les six modules de la couche cliente sont nommés par leur préoccupation plutôt que par un répertoire. La décision d'arborescence ne rouvre donc pas le plan — elle conditionne la disposition en fichiers du futur code, pas la désignation des modules que ce plan porte déjà. `structure-projets.md § 3` fixe par ailleurs une convention de désignation qui permet aux documents dérivés de nommer un module avant que son nom de projet soit tranché.
...
- **Sous-grain MoSCoW des fiches `US-UC-*`.** Retiré le 2026-09-04 : les quatorze fiches `docs/conception/besoin/user-stories/US-UC-*.md` ne portent plus aucune priorité Must/Should/Could/Won't Have au grain de la story — seul [`moscow.md`](../conception/besoin/vision/moscow.md) a l'autorité du corpus pour cette attribution, sur le précédent de `moscow.md § UC-15` (décision du 2026-09-01, qui posait déjà ce principe pour un cas voisin). La priorité se lit désormais exclusivement au grain du use case, à sa source dans `moscow.md`, que chaque fiche cite en renvoi.
