...
[ ZONE DE PRÉSENTATION DES CONSÉQUENCES ]
  ...
  rôle     : expose de façon exhaustive et lisible les effets irréversibles de la
             suppression — campagnes orphelines, suppression physique des notes de
             session personnelles joueur créées par l'utilisateur (y compris celles
             rattachées aux personnages qu'il incarnait dans les campagnes vivantes),
             anonymisation des données nominatives dans les autres tables ;
...
- [UC-10 A4] confirmer la suppression de son compte → le compte est marqué supprimé,
  les données nominatives sont anonymisées, les notes de session personnelles joueur
  créées par l'utilisateur sont supprimées physiquement (NFR-CONF-03) ;
